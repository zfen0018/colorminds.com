...
const ARROW_STYLES = [
	{ value: 'arrow-style-1', label: __( 'Style 1', 'eedee-gutenslider' ) },
	{ value: 'arrow-style-10', label: __( 'Style 2', 'eedee-gutenslider' ) },
];
...
export default ( props ) => {
	...
	const dividerControls = (
		<>
			<BaseControl.VisualLabel>{ __( 'Top Divider', 'eedee-gutenslider' ) }</BaseControl.VisualLabel>
			<Select
				className="eedee-divider-select"
				defaultValue={ DIVIDER_OPTIONS[ 0 ] }
				label={ __( 'Select a top divider', 'eedee-gutenslider' ) }
			...
			/>
			<ColorPalette
				label={ __( 'Background Color', 'eedee-gutenslider' ) }
			...
			/>
			<ToggleControl
				label={ __( 'Flip Vertical', 'eedee-gutenslider' ) }
				checked={ dividers.top.flipY }
				onChange={ ( flipY ) => setAttributes( { dividers: { ...dividers, top: { ...dividers.top, flipY } } } ) }
			/>
			<BaseControl.VisualLabel>{ __( 'Bottom Divider', 'eedee-gutenslider' ) }</BaseControl.VisualLabel>
			<Select
				className="eedee-divider-select"
				defaultValue={ DIVIDER_OPTIONS[ 0 ] }
				label={ __( 'Select a bottom divider', 'eedee-gutenslider' ) }
				options={ Object.values( DIVIDER_OPTIONS ) }
	...
	const responsiveSliderHeightBreakpoints = [
		{
			...
			componentProps: {
				...
				label: ( <span> { __( 'Minimum Slider Height', 'eedee-gutenslider' ) } <small>({ __( 'Desktop', 'eedee-gutenslider' ) })</small></span> ),
		...
		},
		{
			...
			componentProps: {
				...
				isFullScreen: isFullScreen,
				label: ( <span> { __( 'Minimum Slider Height', 'eedee-gutenslider' ) } <small>({ __( 'Tablet', 'eedee-gutenslider' ) })</small></span> ),
		...
		},
		{
			...
			componentProps: {
				...
				isFullScreen: isFullScreen,
				label: ( <span> { __( 'Minimum Slider Height', 'eedee-gutenslider' ) } <small>({ __( 'Mobile', 'eedee-gutenslider' ) })</small></span> ),
	...
	return (
		<InspectorControls>
			<PanelBody
				title={ __( 'General', 'eedee-gutenslider' ) }
				className="gutenslider-controls gutenslider-controls-general eedee-icon-panel"
				icon={ <FeatherIcon.Settings color={ '#000' } size={ 18 } /> }
			>
				<BaseControl help={ __( 'Choose the transition between slides', 'eedee-gutenslider' ) }>
					<BaseControl.VisualLabel>{ __( 'Animation', 'eedee-gutenslider' ) }</BaseControl.VisualLabel>
				...
				<BaseControl help={ __( 'Can each slide have its own content or are child blocks fixed on top of all slides?', 'eedee-gutenslider' ) }>
					<BaseControl.VisualLabel>{ __( 'Slide Mode', 'eedee-gutenslider' ) }</BaseControl.VisualLabel>
				...
				{ ( animation === 'slide' || animation === 'coverflow' ) &&
					<RangeControl
						label={ ( <span> { __( 'Slides to Show', 'eedee-gutenslider' ) } </span> ) }
						value={ slidesToShow }
						...
						help={ slidesToShow === 0 ? __( 'Auto Slides per view will set the slider to the height you set in the "Min Slider Height" setting, and disable automatic height recognition. So check yourself that the content does not overflow the slides.', 'eedee-gutenslider' ) : null }
						min={ 0 }
						max={ 1 }
						step={ 1 }
						marks={ [
							{
								value: 0,
								label: __( 'auto', 'eedee-gutenslider' ),
							},
							{
								value: 1,
								label: '1',
				...
				{ ( animation === 'coverflow' || animation === 'slide' ) && slidesToShow !== 1 && <ToggleControl
					label={ __( 'Centered Slides', 'eedee-gutenslider' ) }
					checked={ centeredSlides }
					onChange={ ( val ) => setAttributes( { centeredSlides: val } ) }
				/> }
				{ false && <ToggleControl
					label={ __( 'Zoom', 'eedee-gutenslider' ) }
					checked={ zoom }
			...
			{ autoplay && <PanelBody
				title={ __( 'Autoplay', 'eedee-gutenslider' ) }
			...
			>
				<SelectControl
					label={ __( 'Arrow Style', 'eedee-gutenslider' ) }
					value={ arrowStyle }
				...
				<RangeControl
					...
					step={ 1 }
					value={ arrowSize }
					label={ ( <span>{ __( 'Arrow Size', 'eedee-gutenslider' ) }</span> ) }
					onChange={ ( val ) => setAttributes( { arrowSize: val, arrowSizeMd: val, arrowSizeSm: val } ) }
				/>
				<BaseControl>
					<BaseControl.VisualLabel>{ __( 'Arrow Color', 'eedee-gutenslider' ) }</BaseControl.VisualLabel>
					<ColorPalette
						value={ arrowColor }
						colors={ [
			...
			>
				<RangeControl
					value={ dotSize }
					label={ ( <span> { __( 'Progress Size', 'eedee-gutenslider' ) }</span> ) }
				...
				/>
				<ColorPalette
					label={ __( 'Dot Color', 'eedee-gutenslider' ) }
					colors={ [
							{ name: 'black', color: '#000000' },
							{ name: 'white', color: '#ffffff' },
							{ name: 'blue', color: '#007cba' },
				...
				<RangeControl
					value={ dotYOffset }
					label={ ( <span> { __( 'Progress Offset', 'eedee-gutenslider' ) } </span> ) }
			...
			<PanelBody
				title={ __( 'Dividers', 'eedee-gutenslider' ) }
				initialOpen={ false }
				className="gutenslider-controls-dividers eedee-icon-panel"
				icon={ icons.divider }
			>
				{ animation === 'flip' && <Disabled>
					<p>{ __( 'Dividers are disabled if animation is set to flip.', 'eedee-gutenslider' ) }</p>
						{ dividerControls }
				</Disabled> }
				{ animation === 'cube' && <Disabled>
					<p>{ __( 'Dividers are disabled if animation is set to cube.', 'eedee-gutenslider' ) }</p>
						{ dividerControls }
				</Disabled> }
			...
			>
			<BaseControl.VisualLabel>{ __( 'Padding', 'eedee-gutenslider' ) }</BaseControl.VisualLabel>
				<UnitButton
					value={ paddingX }
				...
				<UnitButton
					...
					transformUnits={ true }
				/>
				<BaseControl.VisualLabel>{ __( 'Margin', 'eedee-gutenslider' ) }</BaseControl.VisualLabel>
				<UnitButton
					value={ marginX }
					label={ ( <span>{ __( 'Left + Right', 'eedee-gutenslider' ) }</span> ) }
